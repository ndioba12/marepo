import { Component, OnInit } from '@angular/core';
import Swal from 'sweetalert2';
import { AFFECTATIONS, TYPEDEMATERIEL, ETAT, MATERIELS } from '../../../../../shared/models/constantes-model';
import { Materiel } from '../../../../../shared/models/materiel-model';

@Component({
  selector: 'app-list-referencement',
  templateUrl: './list-referencement.component.html',
  styleUrls: ['./list-referencement.component.css']
})
export class ListReferencementComponent implements OnInit {

  page = 1;
	pageSize = 5;
	collectionSize = AFFECTATIONS.length;
  typeMateriel = TYPEDEMATERIEL;
  etat = ETAT;
  materiels! : Materiel[];

  statusMateriel:any[] = [
    { value:"non reférencié" },
    { value:"reférencié" },
    { value:"affecté" }
  ]

  constructor() {}

  ngOnInit(): void {
    this.refreshMateriels();
  }

  deleteItem() {
    Swal.fire({
      title: 'Êtes-vous sûr de faire cette suppression ?',
      text: "Vous ne pourrez pas revenir en arrière !",
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#1C5EAB',
      cancelButtonColor: '#FF4D4F',
      confirmButtonText: 'Confirmer',
      cancelButtonText: 'Annuler'
    }).then((result) => {
      if (result.isConfirmed) {
        Swal.fire({
          title: 'Supprimé',
          text: "L'élément a été supprimé.",
          icon: 'success',
          timer: 2000,
          showCancelButton: false,
          showConfirmButton: false
        })
      }
    })
  }

  refreshMateriels() {
		this.materiels = MATERIELS.map((materiel, i) => ({ id: i + 1, ...materiel })).slice(
			(this.page - 1) * this.pageSize,
			(this.page - 1) * this.pageSize + this.pageSize,
		);
	}

}
