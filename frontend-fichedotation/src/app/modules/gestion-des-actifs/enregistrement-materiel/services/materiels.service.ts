...
import { AjoutMaterielAchatDTO, DetailMaterielDTO } from '../models/materiel.model';
...
export class MaterielsService {
  ...
  editMateriel(id: number, dto: DetailMaterielDTO): Observable<ResponseApi> {
    return this._http.put<ResponseApi>(
      `${environment.apiUrl}/editImmobilisation/${id}`,
      dto
    );
  }

  deleteMateriel(id: number): Observable<ResponseApi> {
    return this._http.delete<ResponseApi>(`${environment.apiUrl}/deleteImmobilisation/${id}`);
  }
