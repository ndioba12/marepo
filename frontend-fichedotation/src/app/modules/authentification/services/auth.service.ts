...
const TOKEN_KEY = 'FICHE_DOTATION';
...
export class AuthService {
  ...
  signup$ = (data: any): Observable<ResponseApi> =>
    this.http.post<ResponseApi>(`${environment.apiUrl}/auth/signup`, data);

  authenticateUserWithFirstUrlConnexion = (
    credentials: any
  ): Observable<ResponseApi> =>
    this.http.post<ResponseApi>(
      `${environment.apiUrl}/auth/signinWithFirstUrlConnexion`,
      credentials
    );

  authenticateUserWithForgetPasswordUrlConnexion = (
    credentials: any
  ): Observable<ResponseApi> =>
    this.http.post<ResponseApi>(
      `${environment.apiUrl}/auth/signinWithForgetPasswordUrlConnexion`,
      credentials
    );

  handleForgetPassword = (login: string): Observable<any> =>
    this.http.get(`${environment.apiUrl}/auth/forgotPassword?login=${login}`);

  editUserInfos = (infos: any): Observable<ResponseApi> =>
    this.http.post<ResponseApi>(
      `${environment.apiUrl}/auth/editUserInfos`,
      infos
    );

  editUserPassword = (infos: any): Observable<ResponseApi> =>
    this.http.post<ResponseApi>(
      `${environment.apiUrl}/auth/editUserPassword`,
      infos
    );
  ...
  runTimeoutInterval(user: User): void {
    ...
    const timeInterval =
      expirationDate - todaysDate - TIME_MILLIS_BEFORE_LOGOUT;
  ...
  onLogout(): void {
    ...
    this.router.navigate(
      [this.route.snapshot.queryParams['redirect'] || '/auth/login'],
      { replaceUrl: true }
    );
  }
