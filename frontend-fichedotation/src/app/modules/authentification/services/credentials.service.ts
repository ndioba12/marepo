...
const credentialsKey = 'FICHE_DOTATION';
...
export class CredentialsService {
  ...
  get userProfil(): string {
    return this.userInfos ? this.userInfos?.linkedProfil?.libelle : 'Unknow';
  }

  get username(): string {
    return this.userInfos ? this.userInfos?.email : 'Unknow';
  }

  get userEntity(): string {
    return this.userInfos ? this.userInfos?.linkedEntite?.libelle : 'Unknow';
  }

  get userMatricule(): string {
    return this.userInfos ? this.userInfos?.matricule : 'Unknow';
  }

  get userFonction(): string {
    return this.userInfos ? this.userInfos?.fonction : 'Unknow';
  }

  get userAuthorities(): string[] {
    let roles: any[] = [] ;
    if (this.isAuthenticated()) {
      this._credentials?.authorities.forEach((authority: any) => {
